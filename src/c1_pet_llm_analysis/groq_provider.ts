...
import { LlmWrapper, LlmWrapperSettings, LlmResponse } from '../c1_pet_llm_analysis/llm_wrapper';

/**
 * Groq cloud LLM provider
 *
 * Uses the Groq SDK to communicate with Groq's fast inference API.
 * This is the original provider implementation migrated to the LlmWrapper architecture.
 */
export class GroqProvider extends LlmWrapper {
  ...
  constructor(settings: LlmWrapperSettings) {
    ...
    const apiKey = settings.groqSettings?.apiKey || settings.groqSettings?.apiKey;
    this.model = settings.groqSettings?.model || settings.model;

    this.debug(`GroqProvider constructor called with:`);
    this.debug(`  apiKey: ${apiKey ? 'provided' : 'not provided'}`);
    this.debug(`  model: ${this.model}`);
    this.debug(`  timeout: ${this.timeout}`);
